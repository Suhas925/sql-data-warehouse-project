...
-- Truncating and inserting the transformed data into silver layer from the bronze layer using a stored procedure.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			CASE
				WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid)) -- Remove 'NAS' prefix is present.
				ELSE cid
			END AS CID,
			CASE
